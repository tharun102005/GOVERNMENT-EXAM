...
import { Download, Sparkles, Bookmark, Search, Eye, Filter, Calendar, FileText, CheckCircle2, Lock } from 'lucide-react';
import { Link } from 'react-router-dom';
import { showToast } from '../components/Toast';
import { getStoredPdfRegistry } from '../data/pdfRegistry';
import { downloadRealPdf } from '../utils/pdfDownloader';

export default function PYQPage() {
  const [selectedYear, setSelectedYear] = useState('All');
  const [selectedExam, setSelectedExam] = useState('All');
  const [selectedPyq, setSelectedPyq] = useState(null);
  const [downloadProgress, setDownloadProgress] = useState({}); // { [id]: number }

  const pyqList = getStoredPdfRegistry();
  ...
  const handleDownload = async (p) => {
    if (!p.isPublished) {
      showToast('PDF not available for this exam.', 'warning');
      return;
    }

    try {
      setDownloadProgress((prev) => ({ ...prev, [p.id]: 10 }));

      await downloadRealPdf(p.id, (progress) => {
        setDownloadProgress((prev) => ({ ...prev, [p.id]: progress }));
      });

      showToast(`Downloaded ${p.title} (${p.fileSize})`, 'success');
    } catch (err) {
      showToast(err.message || 'PDF not available.', 'warning');
    } finally {
      setTimeout(() => {
      ...
      <main className="flex-1 max-w-[1440px] mx-auto px-6 lg:px-12 py-10 w-full">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Download solved PDFs directly to your computer or practice shift-wise papers online.
          </p>
        </div>
        ...
        <div className="space-y-4">
          {filtered.map((p) => {
            const isDownloading = downloadProgress[p.id] !== undefined;
            const progressVal = downloadProgress[p.id] || 0;
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-400">📥 {p.downloads.toLocaleString()} Downloads</span>
                    </div>
                    <h3 className="font-extrabold text-lg text-slate-900 dark:text-white mt-1">{p.title}</h3>
                    <p className="text-xs text-slate-500 mt-1">
                      {p.subject} • PDF File ({p.fileSize || '3.5 MB'})
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                  ...
                  {p.isPublished ? (
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs flex items-center gap-1.5 shadow-md shadow-blue-500/20 transition cursor-pointer disabled:opacity-60"
                    >
                      <Download className="w-3.5 h-3.5" />
                      <span>{isDownloading ? `Downloading... ${progressVal}%` : 'Download PDF'}</span>
                    </button>
                  ) : (
                    <button
                      disabled
                      className="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 font-bold text-xs flex items-center gap-1 cursor-not-allowed opacity-60"
                    >
